...
class Marche:
   ...
   def __init__(self, nom_marche, x_ligne=50, y_colonne=50, marche_id=None, grille=None):
      ...
      self.grille = grille if grille is not None else [[False for _ in range(self.x_ligne)] for _ in range(self.y_colonne)] # False = pas de marchand, True = marchand
   ...
   def occup_stand(self, x, y):
      ...
      if self.stand_available(x, y):
         ...
         try:
            ...
            raise Exception(f"Erreur lors de l'occupation du stand : {e}")
      else:
   ...
   def update_grille(self):
      ...
      try:
         marche_existant = collection.find_one({'marche_id': self.marche_id})
         if not marche_existant:
            raise Exception(f"Aucun marché trouvé avec l'ID {self.marche_id}.")
         
         result = collection.update_one(
            {'marche_id': self.marche_id},
            {'$set': {'grille': self.grille}}
         )
         if result.matched_count == 0:
            raise Exception(f"Aucun marché trouvé avec l'ID {self.marche_id}.")
         if result.modified_count == 0:
            if marche_existant.get('grille') == self.grille:
               rprint("[bold yellow]Le stand a été associé au marchand et la grille mise à jour.[/bold yellow]")
            else:
               raise Exception("La grille n'a pas été mise à jour dans la base de données.")
      
      except PyMongoError as e:
         raise Exception(f"Erreur lors de la mise à jour du marché : {e}")
      finally:
         db.client.close()
   ...
   def get_one(marche_id):
      ...
      collection = db['marches']
      try:
         marche = collection.find_one({'marche_id': marche_id})
         if not marche:
   ...
   @staticmethod
   def from_dict(data):
      """Recree une instance Marche"""
      return Marche(
         nom_marche=data['nom_marche'],
         x_ligne=data['x_ligne'],
         y_colonne=data['y_colonne'],
